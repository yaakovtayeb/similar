...
drop table yaakovt.estimationAcc;
CREATE EXTERNAL TABLE IF NOT EXISTS yaakovt.estimationAcc(
country         double,
type            string,
error           double,
sitesize        double,
...
ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t'
LINES TERMINATED BY '\n'
LOCATION '/user/assaf.zohar/accuracy_tests';
...
MSCK REPAIR TABLE yaakovt.estimationAcc;
...
SELECT  a.countryName,
        ...
        ${month},
        ${year}
FROM yaakovt.estimationAcc AS b
...
SELECT  a.countryName,
        ...
        ${month},
        ${year}
FROM yaakovt.estimationAcc AS c
...
hive -e "set hive.cli.print.header=true; SELECT * FROM yaakovt.accuracyReport WHERE YEAR=17 AND MONTH=5;">/home/yaakov.tayeb/output/AccReport1705.tsv;
