...
for country in [152]:
    for date in set(data["date"]):
        tmpdata = data.loc[(data['country'] == country) & (data['date'] == date)]
        # filter the data
        tmpdata = tmpdata.loc[(data['category'] == "Internet_and_Telecom") & (data["ga_mobile"]>100000)]
        err = tmpdata[(data['ga_mobile'] > sitesizeforerror)][['ga_mobile', 'sw_mobile']].values
        x = tmpdata["ga_mobile"] # GA data for a specific month
        y = tmpdata["sw_mobile"] # SW
        sites_text = tmpdata["site"].values
        categories = tmpdata["category"].values
        colors = categoryColor(categories)
        ...
        plotTitle = "%d Mobile Accuracy Check. Month: %s. R: %2f, d: %.2f" % (country, date, R_list[-1], delta_list[-1])
        layout = Layout(
            title=plotTitle,
            xaxis=dict(
