 add jar hdfs:////similargroup/jarsobile/daily/mobile/2016-04-30/2016-05-01-06-47-58-254/mobile-web/JRI-1.jar;
...
DROP TABLE yaakovt.topsites_for_source;
CREATE EXTERNAL TABLE IF NOT EXISTS yaakovt.topsites_for_source (
    site string, 
    rank int
    ) ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t' LINES TERMINATED BY '\n' LOCATION '/user/yaakov.tayeb/outputs/topsites/table/';

DROP TABLE yaakovt.topsites_by_source;
CREATE EXTERNAL TABLE IF NOT EXISTS yaakovt.topsites_by_source (
    site string, 
    paid double,
    month int,
    year int
    ) ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t' LINES TERMINATED BY '\n' LOCATION '/user/yaakov.tayeb/outputs/topsites/results/';

INSERT overwrite TABLE yaakovt.topsites_by_source
SELECT top3.site,
       top3.v/(sum(top3.v) over ()),
       top3.month,
       top3.year
FROM
        (SELECT top.site,
                sum(top.estimatedvisits) AS v,
                top.month,
                top.YEAR
         FROM analytics.snapshot_estimated_totals_sr top
         WHERE top.YEAR=16
                 AND top.MONTH=12
                 AND top.country=643
                 AND getTopSpecialReferrerUDF(top.refid)=1
                 AND top.refflag=0
                 AND top.site IN
                         (SELECT geturlwithkeysuffixudf(top2.site) FROM yaakovt.topsites_for_source top2)
         GROUP BY top.site,
                  top.year,
                  top.month) top3;

hive -e "set hive.cli.print.header=true; SELECT * FROM yaakovt.topsites_by_source;">/home/yaakov.tayeb/output/topsites_by_source.All.paid.tsv;
hive -e "set hive.cli.print.header=true; SELECT * FROM yaakovt.topsites_by_source;">/home/yaakov.tayeb/output/topsites_by_source.All.organic.tsv;
hive -e "set hive.cli.print.header=true; SELECT * FROM yaakovt.topsites_by_source;">/home/yaakov.tayeb/output/topsites_by_source.Travel.organic.tsv;
hive -e "set hive.cli.print.header=true; SELECT * FROM yaakovt.topsites_by_source;">/home/yaakov.tayeb/output/topsites_by_source.Travel.paid.tsv;
hive -e "set hive.cli.print.header=true; SELECT * FROM yaakovt.topsites_by_source;">/home/yaakov.tayeb/output/topsites_by_source.Shopping.paid.tsv;
hive -e "set hive.cli.print.header=true; SELECT * FROM yaakovt.topsites_by_source;">/home/yaakov.tayeb/output/topsites_by_source.Shopping.organic.tsv;

--hive -e "set hive.cli.print.header=true; SELECT * FROM yaakovt.topsites_for_source;">/home/yaakov.tayeb/output/topsites_for_source.tsv;


--SELECT CONCAT(top.site,'*') as site,
--select count(site) from analytics.snapshot_estimated_totals_incoming_referral where getTopSpecialReferrerUDF(refid)=1 AND refflag=TRUE and month=2 and year=17;
-- snapshot_estimated_total_sr / HBase traffic_distro
