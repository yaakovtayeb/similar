...
set hivevar:weekbegins_2y = 'qa15';
set hivevar:weekends_2y = 'qa16';
set hivevar:day1_2y = qa17;
set hivevar:month1_2y = qa18;
set hivevar:day2_2y = qa19;
set hivevar:month2_2y = qa20;
set hivevar:year_2y = qa21;
...
inner join yaakovt.jlsites as a on (b.site=a.site)
INNER JOIN (select c.site, sum(c.estimatedvisits) as visits from analytics.daily_estimated_values as c WHERE c.country="999" and c.year = ${lyyear} and ((c.month = ${lymonth1} and c.day >= ${lyday1} and c.day<= (${lyday1}+6)) or (c.month=${lymonth2} and c.day>=(${lyday2}-6) and c.day<=${lyday2})) group by c.site) c ON (c.site = a.site)
...
(SELECT
        a.site,
        'WorldWide',
        ${weekbegins_2y} as date1,
        ${weekends_2y} as date2,
        sum(if(getTopSpecialReferrerUDF(b.specialref)=5, 1, 0)) AS direct,
        sum(if(getTopSpecialReferrerUDF(b.specialref)=4, 1, 0)) AS displayads,
        sum(if(getTopSpecialReferrerUDF(b.specialref)=3, 1, 0)) AS mail,
        sum(if(getTopSpecialReferrerUDF(b.specialref)=1 and reftype="ORGANIC", 1, 0)) AS organic_search,
...
FROM ds.parquet_visits as b
inner join yaakovt.jlsites as a on (b.site=a.site)
INNER JOIN (select c.site, sum(c.estimatedvisits) as visits from analytics.daily_estimated_values as c WHERE c.country="999" and c.year = ${year_2y} and ((c.month = ${month1_2y} and c.day >= ${day1_2y} and c.day<= (${day1_2y}+6)) or (c.month=${month2_2y} and c.day>=(${day2_2y}-6) and c.day<=${day2_2y})) group by c.site) c ON (c.site = a.site)
WHERE b.year = ${year_2y} and b.country<999
and ((b.month = ${month1_2y} and b.day>=${day1_2y} and b.day<=(${day1_2y}+6)) or (b.month = ${month2_2y} and b.day>=(${day2_2y}-6) and b.day<=${day2_2y}))
group by a.site, c.visits
) traffic;